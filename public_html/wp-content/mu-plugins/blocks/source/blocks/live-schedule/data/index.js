...
export function getCurrentSessions( { sessions, tracks } ) {
	const nowTimestamp =
		window.WordCampBlocks[ 'live-schedule' ].nowOverride || Date.now();
		  ...
		  [
				{
					track: {},
					sessions: reverse(
						sortBy( sessions, 'meta._wcpt_session_time' )
					),
				},
		  ];
	...
	return trackListWithSessions
		.map( ( { track, sessions: sessionsInTrack } ) => {
			...
			const firstSessionTimestamp =
				firstSession.meta._wcpt_session_time * 1000;
