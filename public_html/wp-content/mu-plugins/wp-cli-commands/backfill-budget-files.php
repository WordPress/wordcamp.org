 ...
 * @param int|null $blog_id Defaults to the current site.
 *
 * @return object[] `ID`, `file`, `post_date`, `post_status`, `marked` and `delete_listed` per attachment.
 */
function get_unattached_candidates( $blog_id = null ) {
	...
	$candidates = $wpdb->get_results( $wpdb->prepare(
		"SELECT file.ID, file.post_date, file.post_status, name.meta_value AS file, marker.meta_id AS marked,
			delete_marker.meta_id AS delete_listed
		FROM {$prefix}posts AS file
		...
		LEFT JOIN {$prefix}postmeta AS marker
			ON marker.post_id = file.ID AND marker.meta_key = %s
		LEFT JOIN {$prefix}postmeta AS delete_marker
			ON delete_marker.post_id = file.ID AND delete_marker.meta_key = %s
		WHERE file.post_type = 'attachment'
...
class Command extends WP_CLI_Command {
	 ...
	 * `post_date` and `_wp_attached_file` are still on a request on another site. A site stays in the report
	 * while it holds a copy that no run has recorded, even with nothing left to mark.
	 *
	 * ## EXAMPLES
	 *
	...
	public function scan( $args, $assoc_args ) {
		...
		foreach ( $sites as $site ) {
			...
			/*
			 * A marked file with no `BUDGET_FILE_DELETE_META_KEY` is still work if the network view makes it a
			 * copy, and only `report_scan()` knows that, since a copy's original can sit on a later blog ID.
			 */
			$is_unlisted = fn( $candidate ) => $candidate->marked && ! $candidate->delete_listed;
			$unlisted    = array_values( array_filter( $unattached_candidates, $is_unlisted ) );

			if ( ! $unmarked && ! $unattached && ! $unlisted ) {
				continue;
			}
		...
		$outstanding = $this->report_scan( $reports, $attached );
		...
		WP_CLI::success( sprintf( '%d of %d sites have payment files to migrate.', $outstanding, count( $sites ) ) );
	}
	 ...
	 *
	 * @param array[] $reports  One entry per site that may still have work outstanding.
	 * @param array   $attached Blog IDs per `file_key()` of every file still on a request.
	 *
	 * @return int The sites printed, which is fewer than `$reports` when a site was only there for a file that
	 *             turned out not to be a copy.
	 */
	protected function report_scan( $reports, $attached ) {
		...
		foreach ( $reports as $report ) {
			...
			foreach ( array_merge( $report['unattached'], $report['unlisted'] ) as $candidate ) {
			...
			// A site kept only for its already-marked files is done unless those are copies still to record.
			if ( ! $report['attached'] && ! $report['unattached'] && ! $copies ) {
				continue;
			}
