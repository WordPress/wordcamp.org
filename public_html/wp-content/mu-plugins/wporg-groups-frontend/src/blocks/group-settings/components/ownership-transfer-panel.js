...
const STATUS_LABELS = {
	pending_acceptance: __( 'Awaiting candidate acceptance', 'wordcamporg' ),
	pending_approval: __( 'Accepted — awaiting network admin approval', 'wordcamporg' ),
};
...
function assertUsable( data ) {
	if ( ! data || typeof data !== 'object' ) {
		throw new Error( __( 'Could not load ownership transfer status.', 'wordcamporg' ) );
	}
...
export default function OwnershipTransferPanel() {
	...
	useEffect( () => {
		fetchState().catch( ( err ) => {
			setNoticeType( 'error' );
			setNotice( err.message || __( 'Could not load ownership transfer status.', 'wordcamporg' ) );
			setLoading( false );
		} );
	}, [ fetchState ] );
	...
	const runAction = useCallback( ( path, data ) => {
		...
		apiFetch( { path: `/wporg-groups/v1/ownership-transfer/${ path }`, method: 'POST', data } )
			...
			.catch( ( err ) => {
				setNoticeType( 'error' );
				setNotice( err.message || __( 'That action could not be completed.', 'wordcamporg' ) );
			} )
	...
	return h(
		...
		h( 'h3', { className: 'wporg-settings-tab__section-title' }, __( 'Group ownership', 'wordcamporg' ) ),
		notice &&
		...
		pending &&
			h(
				...
				sprintf(
					...
					__( 'Transfer from %1$s to %2$s: %3$s', 'wordcamporg' ),
		...
		pending &&
			...
			h(
				...
				h(
					...
					__( 'Cancel transfer', 'wordcamporg' )
				)
			),
				...
				candidates.length === 0
					...
					: [
						needsFromUser &&
							h( SelectControl, {
								key: 'from',
								label: __( 'Current owner being replaced', 'wordcamporg' ),
								value: fromUserId,
								options: [ { label: __( 'Select an owner…', 'wordcamporg' ), value: '' } ]
						...
						h( SelectControl, {
							key: 'candidate',
							label: __( 'Transfer ownership to', 'wordcamporg' ),
							value: candidateId,
							options: [ { label: __( 'Select a member…', 'wordcamporg' ), value: '' } ]
						...
						h(
							...
							__( 'Request transfer', 'wordcamporg' )
						),
					]
			),
		...
		historyEntries.length > 0 &&
			h(
				...
				h( 'summary', {}, __( 'Recent transfers', 'wordcamporg' ) ),
				h(
					'ul',
					{},
