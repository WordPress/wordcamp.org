...
function render_subpage() {
	...
	$comment_status = sanitize_text_field( wp_unslash( $_GET['comment_status'] ?? '' ) );
...
function filter_list_table_query_args( $args ) {
	...
	$requested_status = sanitize_text_field( wp_unslash( $_GET['comment_status'] ?? '' ) );
	...
	$helpful = sanitize_text_field( wp_unslash( $_GET['helpful'] ?? '' ) );
	if ( $helpful ) {
		$meta_query = array(
			'key'   => 'helpful',
			'value' => 1,
...
function filter_list_table_views( $views ) {
	...
	$status          = sanitize_text_field( wp_unslash( $_GET['comment_status'] ?? '' ) );
