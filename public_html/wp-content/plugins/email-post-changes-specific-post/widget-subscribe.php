...
class EPCSP_SubscribeWidget extends WP_Widget {
	 ...
	 * @return bool
	 */
	protected function is_supported_epc_post_type( $post_type ) {
		$supported = false;
	 ...
	 */
	protected function render_subscription_form() {
		?>

		<form id="epcsp_subscribe" name="epcsp_subscribe" action="" method="POST">
			<label for="epcsp_subscribe_address"><?php esc_html_e( 'Email Address:', 'wordcamporg' ); ?></label>
	 ...
	 * @return array
	 */
	protected function subscribe_visitor_to_post( $email, $post_id ) {
		$errors  = array();
	...
	public function form( $instance ) {
		...
		<?php if ( ! $this->is_epc_enabled() ) : ?>
			<div class="error inline">
				...
				The 'Enable' setting on <a href="<?php echo esc_url( admin_url( 'options-general.php?page=email_post_changes' ) ); ?>">the Email Post Changes settings page</a> is disabled.
				No emails will be sent until it is enabled.
			</div>
		<?php endif; ?>
	 ...
	 * @return bool
	 */
	protected function is_epc_enabled() {
		$enabled = false;
